...
__SYSCALL_DEFINEx(1, ftrace, pid_t, pid){
	...
	struct task_struct *sibling_struct, *child_struct;
	//struct task_struct *pgid_struct;
	...
	*/
	...
	//Print context swiches, parent process, silbing
	...
	//Search Silbing using parent process
...
static int __init processtracer_init(void){
...
static void __exit processtracer_exit(void){
	syscall_table[__NR_ftrace] = real_ftrace;
	make_ro(syscall_table);//Recover syscall
}

module_init(processtracer_init);
module_exit(processtracer_exit);
MODULE_LICENSE("GPL");
